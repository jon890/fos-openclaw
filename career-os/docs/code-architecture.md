...
│   - 0개 case (plan006 이후 ADR-019, plan016에서 2 case 폐기, │
│     plan017에서 3 case 폐기, plan021에서 1 case 폐기,        │
│     plan022에서 마지막 1 case 폐기 → dispatcher case 0개 도달)│
│   - run_tracked() 헬퍼: tracker 래핑 + 알림 + cost summary  │
│   - plan023에서 command-router 디렉터리 자체 폐기 예정       │
└────────────────┬────────────────────────────────────────────┘
                 │
┌────────────────┴────────────────────────────────────────────┐
│ 트래커 (외부 의존)                                            │
...
│   ├── command-router/                 (dispatcher case 0개 도달 — plan023에서 디렉터리 폐기 예정)
│   │   ├── run_now.sh                  디스패처 본체 (0개 case, plan022에서 마지막 recommend-positions 폐기)
...
| `_shared/lib/mvp_target_schema.ts` | Bun/zod. `config/mvp-target.json` 스키마 검증 단일 출처. `parseMvpTarget()` 함수 + `CoffeechatSchema` 포함 (plan021 ADR-029). |
| `_shared/bin/update_artifacts.py` | `data/generated-artifacts.json` upsert (당분간 Python 유지, 별도 plan). |
| `zod` (npm) | TypeScript runtime 스키마 검증. `mvp_target_schema.ts`에서 도입 (plan021 ADR-029). `package.json`에 의존성 추가됨. |
...
case "$MODE" in
  # dispatcher case 0개 — plan022에서 마지막 recommend-positions 폐기 (ADR-030)
  # 모든 진입점은 native skill:
  #   /position-recommender (ADR-030, plan022)
  #   /interview-coffeechat-prep (ADR-029, plan021)
  #   /interview-prep-analyzer (ADR-027, plan017)
  *)
