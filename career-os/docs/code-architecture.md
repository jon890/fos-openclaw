...
│   - 1개 case 분기 (plan006 이후 ADR-019, plan016에서 2 case 폐기,│
│     plan017에서 3 case 폐기, plan021에서 1 case 폐기 → native) │
...
│   ├── command-router/
│   │   ├── run_now.sh                  디스패처 본체 (1개 case — recommend-positions)
...
case "$MODE" in
  recommend-positions)
    ...
    ;;
  # interview-coffeechat-prep: claude -p "/interview-coffeechat-prep" (native, plan021 ADR-029)
