...
**적용**: `apartment/config/focus-unit.json` (complexLocation 키 추가), `apartment/skills/apartment-daily-report/scripts/run_report.sh:21-25` (env default 폐기 + json read). JSON 읽기 헬퍼 구현 언어 결정은 ADR-003.
...
- skill 디렉터리 안 ts (`apartment/skills/apartment-daily-report/scripts/`) — apartment 다중 collect가 공유 가능성 있어 워크스페이스 레벨이 더 정합.
