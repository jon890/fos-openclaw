...
`~/ai-nodes`는 단일 프로젝트가 아닌 **멀티 워크스페이스 컨테이너**다. 각 최상위 디렉터리는 독립 작업 영역으로 자체 skills · data · logs · config · docs를 가진다. 워크스페이스는 서로 격리되며 다른 워크스페이스의 자산을 교차 참조하지 않는다.
...
Claude Code는 프로젝트 루트의 `CLAUDE.md`를 자동으로 컨텍스트에 로드한다. ai-nodes 워크스페이스의 정식 가이드 파일 이름은 `AGENTS.md`이므로, `CLAUDE.md`는 심링크로 두 이름을 연결한다.
